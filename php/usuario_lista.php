<?php
    ...
    if(isset($busqueda) && $busqueda!=""){
        $consulta_datos="SELECT * FROM usuarios WHERE ((usuario_id!='".$_SESSION['id']."') AND
        ...
        $consulta_total="SELECT COUNT(usuario_id) FROM usuarios WHERE ((usuario_id!='".$_SESSION['id']."') AND
        (usuario_nombre LIKE '%$busqueda%' OR usuario_apellido LIKE '%$busqueda%' 
        OR usuario_usuario LIKE '%$busqueda%' OR usuario_email LIKE '%$busqueda%')) 
        ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";
    ...
    if($total>=1 && $pagina<=$Npaginas){
        $tabla.='<p class="has-text-right">Mostrando usuarios <strong>'.$pag_inicio.'</strong> al <strong>'.$pag_final.
